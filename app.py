...
def login_gate() -> bool:
    ...
    with m:
        st.markdown("<div class='brand' style='justify-content:center'>"
                    "<div class='brand-logo'>🔐</div><div class='brand-name'>"
                    "Admin Portal</div></div><br>", unsafe_allow_html=True)
        if ss.tries >= 5:
            st.error("Bohat zyada ghalat koshishein. App restart karein.")
            return False
        with st.form("lg"):
            ...
            if st.form_submit_button("Login", type="primary", use_container_width=True):
                ...
                want_u = cfg.get("username", "admin")
                want_h = cfg.get("password_sha256", "")
                got_h = hashlib.sha256(p.encode()).hexdigest()
                if hmac.compare_digest(u.strip(), want_u) and want_h and \
                        hmac.compare_digest(got_h, want_h):
                    ss.admin_ok = True
                    st.rerun()
                else:
                    ss.tries += 1
                    st.error(f"Ghalat credentials. ({5 - ss.tries} koshishein baqi)")
        st.caption("Hash banane ke liye: python -c \"import hashlib;"
...
with tabs[0]:
    ...
    if sale:
        ...
        empty("Abhi kisi product par sale nahi. Sale price add karein — banner "
              "automatic slide karega.")
...
with tabs[1]:
    ...
    with left:
        ...
        with st.form("np", clear_on_submit=True):
            ...
            sale = p2.number_input("Sale price (0 = no sale)", 0.0, step=50.0)
            ...
            st.markdown("**Images — max 5 (koi bhi image type)**")
            ...
            if st.form_submit_button("💾 Save & Publish", type="primary",
                ...
                if len(t.strip()) < 3 or cat == "—" or price <= 0:
                    ...
                    db.save_product({
                        ...
                        "sale_price": float(sale) if sale > 0 else None,
    ...
    with right:
        ...
        if not plist:
            ...
            pick = st.selectbox("Select", plist,
            ...
            with st.form("ep"):
                ...
                sale = e2.number_input("Sale price", 0.0,
                ...
                extra = st.file_uploader("Nayi images add karein", accept_multiple_files=True,
                                         type=["png", "jpg", "jpeg", "webp", "gif", "bmp", "avif"])
        ...
        with st.form("nc", clear_on_submit=True):
            ...
            if st.form_submit_button("💾 Add", type="primary", use_container_width=True):
                if len(n.strip()) < 2:
                    ...
                    st.success("Category live ho gayi — user ko turant nazar aayegi.")
                    st.rerun()
    with b:
        ...
        for c in cats:
            r = st.columns([2.4, 1.1, 1, 1], vertical_alignment="center")
...
with tabs[3]:
    ...
    with a:
        with st.form("nb", clear_on_submit=True):
            ...
            c1, c2 = st.columns(2)
            g1 = c1.color_picker("Gradient from", "#0f172a")
            g2 = c2.color_picker("Gradient to", "#e11d48")
    ...
    with b:
        for bn in db.get_banners(active_only=False):
            r = st.columns([3, 1, 1], vertical_alignment="center")
            r[0].markdown(f"**{e(bn.get('title'))}** — {e(bn.get('subtitle'))}")
            if r[2].button("🗑", key=f"db{bn['id']}"):
                db.delete_banner(bn["id"])
                st.rerun()

# ================================================================= ORDERS
with tabs[4]:
    f = st.selectbox("Filter", ["all", "new", "confirmed", "shipped",
                                "delivered", "cancelled"])
        ...
        with st.expander(f"#{o['order_no']} • {o['customer_name']} • "
            ...
            with c1:
                ...
                st.dataframe([{"Item": i["title"], "Qty": i["qty"],
                             ...
                             use_container_width=True, hide_index=True)
            with c2:
                stt = st.selectbox("Status", ["new", "confirmed", "shipped",
                                              "delivered", "cancelled"],
                                   index=["new", "confirmed", "shipped", "delivered",
                                          "cancelled"].index(o["status"]),
                                   key=f"s{o['id']}")
...
with tabs[6]:
    ...
    with st.form("stg"):
        ...
        c1, c2 = st.columns(2)
        df = c1.number_input("Delivery fee", 0.0, value=float(S.get("delivery_fee") or 0),
                             step=50.0)
        fo = c2.number_input("Free delivery over", 0.0,
                             value=float(S.get("free_over") or 0), step=500.0)
        an = st.text_area("Announcement (top scrolling bar)", S.get("announcement", ""),
                          height=80)
