"""Theme + CSS. Do rang alag alag kaam karte hain:
   --act  = ACTION   (buttons, links, active pills)   -> indigo
   --sale = URGENCY  (discount aur sale badges only)  -> red
"""
...
:root{
  ...
  --act:#4338ca;      /* ACTION  — buttons, hover, active pills */
  --act2:#6d28d9;     /* action gradient partner */
  --sale:#e11d48;     /* URGENCY — sirf discount / sale */
  --ok:#16a34a;
...
.brand-logo{width:42px;height:42px;border-radius:12px;display:grid;place-items:center;
  background:linear-gradient(135deg,var(--act),var(--act2));color:#fff;font-size:20px;}
.brand-name{font-family:'Plus Jakarta Sans';font-weight:800;font-size:1.28rem;
  line-height:1.1;color:var(--ink);}
.brand-tag{font-size:.72rem;color:#475569;}
...
.hero-kicker{display:inline-block;background:rgba(255,255,255,.18);
   border:1px solid rgba(255,255,255,.3);padding:4px 12px;border-radius:99px;
...
.hero-old{opacity:.75;text-decoration:line-through;font-weight:500;font-size:.95rem;
   margin-left:8px;}
.hero-off{background:#fff;color:var(--sale);font-weight:800;border-radius:99px;
...
.hero-dots i{width:8px;height:8px;border-radius:99px;background:rgba(255,255,255,.55);
   display:block;}
@media(max-width:760px){.hero-img{display:none}.hero-h{font-size:1.35rem}
...
.sec em{font-style:normal;font-size:.76rem;color:#475569;}

/* ---------- product card ---------- */
.pcard{background:#fff;border:1px solid var(--line);border-radius:var(--r);
   overflow:hidden;transition:.22s;height:100%;}
.pcard:hover{transform:translateY(-4px);box-shadow:var(--shadow);border-color:#c7cbdb;}
.pimg{position:relative;width:100%;padding-top:88%;
   background:#eef1f6 center/cover no-repeat;}
.pbadge{position:absolute;top:9px;left:9px;background:var(--sale);color:#fff;
...
.pcat{font-size:.7rem;color:#475569;text-transform:uppercase;letter-spacing:.06em;}
...
.pold{color:#64748b;text-decoration:line-through;font-size:.8rem;margin-left:6px;}
...
/* ---------- buttons ---------- */
.stButton>button{border-radius:11px;font-weight:600;font-size:.84rem;
   border:1px solid var(--line);}
.stButton>button:hover{border-color:var(--act);color:var(--act);}

/* ---------- product detail ---------- */
.dmain{width:100%;padding-top:74%;border-radius:18px;
   background:#eef1f6 center/cover no-repeat;border:1px solid var(--line);}
.hl{background:#fff;border:1px solid var(--line);border-radius:12px;
   padding:10px 14px;margin:6px 0;font-size:.87rem;}
.kv{display:flex;justify-content:space-between;padding:7px 0;
   border-bottom:1px dashed var(--line);font-size:.9rem;}
.tot{font-weight:800;font-size:1.15rem;color:var(--sale);}
...
.bub.u{background:linear-gradient(135deg,var(--act),var(--act2));color:#fff;
   margin-left:auto;border-bottom-right-radius:4px;}
.bub.a{background:#f1f5f9;color:var(--ink);border-bottom-left-radius:4px;}
.bub small{display:block;opacity:.75;font-size:.66rem;margin-top:3px;}

/* ---------- floating whatsapp (brand green — isay na badlein) ---------- */
.wa{position:fixed;right:20px;bottom:88px;z-index:9999;background:#25D366;
   color:#fff!important;text-decoration:none!important;font-weight:700;font-size:.86rem;
   padding:11px 17px;border-radius:99px;box-shadow:0 8px 22px rgba(37,211,102,.45);
   display:flex;gap:8px;align-items:center;}
.wa:hover{transform:scale(1.05);}

.empty{text-align:center;padding:46px 12px;color:#475569;background:#fff;
